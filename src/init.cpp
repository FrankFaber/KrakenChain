...
public:
    ...
    // KrakenChain Specific WalletInitInterface InitCoinJoinSettings
 ...
 * between krakenchaind, and krakenchain-qt and non-server tools.
*/
...
void PrepareShutdown()
{
    ...
    RenameThread("krakenchain-shutoff");
...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-sporkaddr=<krakenchainaddress>", "Override spork address. Only useful for regtest and devnet. Using this on mainnet or testnet will ban you.", false, OptionsCategory::DEBUG_TEST);
    ...
    gArgs.AddArg("-platform-user=<user>", "Set the username for the \"platform user\", a restricted user intended to be used by KrakenChain Platform, to the specified username.", false, OptionsCategory::MASTERNODE);
...
{
    const std::string URL_SOURCE_CODE = "<http://platform.krakenbit.net>";
    const std::string URL_WEBSITE = "<https://krakenbit.net>";
...
void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("krakenchain-loadblk");
...
/** Sanity checks
 *  Ensure that KrakenChain Core is running in a usable environment with all
...
{
    // Make sure only a single KrakenChain Core process is using the data directory.
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if KrakenChain Core is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if KrakenChain Core is started while in a temporary directory.\n",
        ...
        do {
            ...
            try {
                ...
                if (!fDisableGovernance && !fTxIndex
                   && chainparams.NetworkIDString() != CBaseChainParams::REGTEST) { // TODO remove this when pruning is fixed. See http://platform.krakenbit.net/pull/1817 and http://platform.krakenbit.net/pull/1743
    ...
    // ********************************************************* Step 10c: schedule KrakenChain-specific tasks
