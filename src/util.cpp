...
const char * const BITCOIN_CONF_FILENAME = "krakenchain.conf";
const char * const BITCOIN_PID_FILENAME = "krakenchaind.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "krakenchaind -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\KrakenChain
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\KrakenChain
    // Mac: ~/Library/Application Support/KrakenChain
    // Unix: ~/.krakenchain
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "KrakenChain";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/KrakenChain";
#else
    // Unix
    return pathRet / ".krakenchain";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty krakenchain.conf if it does not excist
