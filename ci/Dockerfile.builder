...
# krakenchain_hash
RUN git clone https://github.com/krakenchainpay/krakenchain_hash
RUN cd krakenchain_hash && python3 setup.py install
...
RUN mkdir /krakenchain-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /krakenchain-src && \
  chown $USER_ID:$GROUP_ID /cache && \
  chown $USER_ID:$GROUP_ID /cache -R
WORKDIR /krakenchain-src
